...
H5Profiler::H5Profiler(const std::string& outfile, int _numProcs) :
    h5file { HighFive::File(outfile,
                            HighFive::File::ReadWrite | HighFive::File::Create | HighFive::File::Truncate,
                            HighFive::MPIOFileDriver(MPI_COMM_WORLD, MPI_INFO_NULL)) },
    numProcs { _numProcs }, step { 0 }, disabled { false }
{
...
void H5Profiler::time(){
    timeStart = MPI_Wtime();
}

void H5Profiler::timePause(const std::string& path){
    timeElapsed[path] += MPI_Wtime() - timeStart;
}
