...
void timeIntegration_BH_par(double t, double delta_t, double t_end, float diam, TreeNode *root, SubDomainKeyTree *s,
    ...
    while (t <= t_end) {
        ...
        if (h5Dump && step % h5DumpEachTimeSteps==0){
            ...
            HighFive::DataSet ranges = h5File.createDataSet<unsigned long>("/lebesgueRanges",
            ...
            HighFive::DataSet key = h5File.createDataSet<unsigned long>("/lebesgueKey", HighFive::DataSpace(N));
