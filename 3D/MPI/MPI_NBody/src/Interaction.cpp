...
Interaction::Interaction() : frictionEnabled { true } {
    LOGCFG.headers = true;
    LOGCFG.level = INFO;
    partSize = NUM_BODIES / world.size();
    partIndex = world.rank() * partSize;
}
...
void Interaction::interactBodies(Body* suns, Body* bods)
{
    ...
    for (int sIndex=0; sIndex<NUM_SUNS; sIndex++)
    {
        ...
        for (int bIndex=0; bIndex<partSize; bIndex++)
        {
            singleInteraction(&suns[sIndex], &bods[bIndex + partIndex], true);
        }
    }
    ...
    for (int bIndex=0; bIndex<partSize; bIndex++)
    {
        if (tree->getOctant().contains(bods[bIndex + partIndex].position))
        {
            treeInteraction(tree, &bods[bIndex + partIndex]);
        }
    }
...
void Interaction::updateBodies(Body* suns, Body* bods)
{
    ...
    for (int bIndex=0; bIndex<partSize; bIndex++) {
        ...
        Body *current = &bods[bIndex + partIndex];
