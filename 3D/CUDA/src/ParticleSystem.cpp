...
ParticleSystem::ParticleSystem(const SimulationParameters p, const int n)
{
    parameters = p;
    step = 0;
    numParticles = n;
    ...
    gpuErrorCheck(cudaMalloc((void**)&d_left, sizeof(float)));
    gpuErrorCheck(cudaMalloc((void**)&d_right, sizeof(float)));
    gpuErrorCheck(cudaMalloc((void**)&d_bottom, sizeof(float)));
    gpuErrorCheck(cudaMalloc((void**)&d_top, sizeof(float)));
    gpuErrorCheck(cudaMemset(d_left, 0, sizeof(float)));
    gpuErrorCheck(cudaMemset(d_right, 0, sizeof(float)));
    gpuErrorCheck(cudaMemset(d_bottom, 0, sizeof(float)));
    gpuErrorCheck(cudaMemset(d_top, 0, sizeof(float)));
    ...
    gpuErrorCheck(cudaMalloc((void**)&d_child, 4*numNodes*sizeof(int)));
...
ParticleSystem& ParticleSystem::operator=(const ParticleSystem &system)
{
    if(this != &system){
        ...
        gpuErrorCheck(cudaFree(d_left));
        gpuErrorCheck(cudaFree(d_right));
        gpuErrorCheck(cudaFree(d_bottom));
        gpuErrorCheck(cudaFree(d_top));
...
const float* ParticleSystem::getOutputBuffer()
{
    return h_output;
}
