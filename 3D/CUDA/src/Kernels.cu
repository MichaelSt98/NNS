...
__global__ void resetArraysKernel(int *mutex, float *x, float *y, float *mass, int *count, int *start,
...
__global__ void computeBoundingBoxKernel(int *mutex, float *x, float *y, float *left, float *right,
                                         float *bottom, float *top, int n)
{
    int index = threadIdx.x + blockDim.x*blockIdx.x;
    int stride = blockDim.x * gridDim.x;
...
__global__ void buildTreeKernel(float *x, float *y, float *mass, int *count, int *start, int *child,
...
__global__ void computeForcesKernel(float* x, float *y, float *vx, float *vy, float *ax, float *ay, float *mass,
                                    int *sorted, int *child, float *left, float *right, int n, float g)
{
    int bodyIndex = threadIdx.x + blockIdx.x*blockDim.x;
    int stride = blockDim.x*gridDim.x;
    int offset = 0;
